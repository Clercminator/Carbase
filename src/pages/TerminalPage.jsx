...
import {
  ...
  Gauge, Home, LayoutDashboard, LogOut, Menu, Plus, Search, SlidersHorizontal, TrendingUp, X,
} from 'lucide-react'
...
const viewCopy = {
  tasaciones: ['Tasaciones', 'Analiza oportunidades y conserva el resultado final de cada decisión.'],
  mercado: ['Mercado', 'Sigue precios, oferta, demanda y liquidez por segmento y región.'],
  inventario: ['Inventario', 'Detecta capital inmovilizado y unidades que requieren un ajuste.'],
  seguimientos: ['Seguimientos', 'Observa publicaciones y recibe cambios relevantes de precio.'],
  alertas: ['Alertas', 'Prioriza variaciones del mercado y eventos de tu inventario.'],
  datos: ['Datos y metodología', 'Revisa cobertura, procedencia, licencias y calidad de las observaciones.'],
}
    ...
    <div className="terminal-page">
      ...
      <div className="terminal-workspace">
        <header className="terminal-topbar">
          ...
          <label className="terminal-search"><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar vehículo, patente o publicación" /><kbd>Ctrl K</kbd></label>
            ...
            <div className="terminal-content">
              ...
              <section className="attention-list">
                {attentionItems.map(({ title, detail, tone, Icon }) => (
                  <button key={title} type="button"><span className={`attention-icon tone-${tone}`}><Icon /></span><span><strong>{title}</strong><small>{detail}</small></span><ChevronRight /></button>
                ))}
              </section>
        ...
        ) : <TerminalView view={activeView} rows={filteredActivity} />}
...
function TerminalView({ view, rows }) {
  const [title, description] = viewCopy[view]
  return <main className="terminal-view"><div className="terminal-view-heading"><div><h1>{title}</h1><p>{description}</p></div><button type="button"><Plus /> Añadir</button></div><div className="view-notice"><LayoutDashboard /><span><strong>Superficie demostrativa</strong> Esta vista está preparada para recibir datos del motor y acciones persistentes cuando se conecte el backend.</span></div><ActivityTable rows={rows} /></main>
}
